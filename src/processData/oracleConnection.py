...
class oracleConnexion(object):
  ...
  def connectDB(self):
    ...
    if numServers == 1:
      ...
      if self.cfg.password == "":
          self.cfg.password  = raw_input("Password de l'usuari {} d'Oracle: ".format(self.cfg.username))
      ...
      if "ssh" in host:
        ...
        DSN = "{}/{}@{}:{}/{}".format(self.cfg.username, self.cfg.password, host["hostname"], host["port"],
                                      self.cfg.sid)
        self.conn = cx_Oracle.connect(DSN)
  ...
  def exists(self, dataset):
    ...
    #return self.conn.cursor().execute(query).fetchone() is not None
    return False
  ...
  def insertImageDataset(self, dataset, fileName, params, labels='anno', imageExt='.jpg', labelsExt='.txt'):
    """
      This method imports the information related to image datasets.
